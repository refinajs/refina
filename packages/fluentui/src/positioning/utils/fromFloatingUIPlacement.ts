import type {
  Side,
  Alignment as FloatingUIAlignment,
  Placement,
...
const getAlignmentMap = (
  position: Position,
): Record<FloatingUIAlignment, Alignment> => {
 ...
 */
export const fromFloatingUIPlacement = (
  placement: Placement,
): { position: Position; alignment?: Alignment } => {
  const { side, alignment: floatingUIAlignment } =
    parseFloatingUIPlacement(placement);
  const position = getPositionMap()[side];
  const alignment =
    floatingUIAlignment && getAlignmentMap(position)[floatingUIAlignment];
